...
def listar_projetos():
    ...
    rows = [{'id': r[0], 'nome': r[1], 'aptos': r[2], 'data_criacao': r[3]} for r in cursor.fetchall()]; conn.close(); return rows
...
def carregar_projeto(projeto_id):
    ...
    col_names = [d[0] for d in cursor.description]; proj_dict = dict(zip(col_names, proj))
    try:
...
def salvar_projeto(projeto_id, dados):
    ...
    for m in dados.get('motores', []):
        ...
        cursor.execute("INSERT INTO motores_projeto (projeto_id, descricao, cv, quantidade) VALUES (?, ?, ?, ?)", (projeto_id, desc, float(m['cv']), int(m['qtd'])))
    cursor.execute("DELETE FROM outras_cargas_projeto WHERE projeto_id = ?", (projeto_id,))
    for c in dados.get('outras_cargas_apt', []):
        cursor.execute("INSERT INTO outras_cargas_projeto (projeto_id, tipo, descricao, potencia, quantidade) VALUES (?, 'apt', ?, ?, ?)", (projeto_id, c[0] if isinstance(c, list) else c['descricao'], float(c[1] if isinstance(c, list) else c['potencia']), int(c[2] if isinstance(c, list) else c['quantidade'])))
    for c in dados.get('outras_cargas_adm', []):
        cursor.execute("INSERT INTO outras_cargas_projeto (projeto_id, tipo, descricao, potencia, quantidade) VALUES (?, 'adm', ?, ?, ?)", (projeto_id, c[0] if isinstance(c, list) else c['descricao'], float(c[1] if isinstance(c, list) else c['potencia']), int(c[2] if isinstance(c, list) else c['quantidade'])))
    ...
    for ac in dados.get('ac_apt', []):
        ...
        cursor.execute("INSERT INTO ac_projeto (projeto_id, tipo, descricao, potencia_w, quantidade, btu, tipo_equipamento, incluir) VALUES (?, 'apt', ?, ?, ?, ?, ?, ?)", (projeto_id, desc, pot, qtd, btu, tipo_eq, incluir))
    for ac in dados.get('ac_adm', []):
        desc = ac.get('descricao', ac.get('desc', '')); pot = float(ac.get('potencia', ac.get('pot', 0))); qtd = int(ac.get('quantidade', ac.get('qtd', 1))); btu = int(ac.get('btu', 0)); tipo_eq = ac.get('tipo_equipamento', ''); incluir = 1 if ac.get('incluir', 1) else 0
        cursor.execute("INSERT INTO ac_projeto (projeto_id, tipo, descricao, potencia_w, quantidade, btu, tipo_equipamento, incluir) VALUES (?, 'adm', ?, ?, ?, ?, ?, ?)", (projeto_id, desc, pot, qtd, btu, tipo_eq, incluir))
