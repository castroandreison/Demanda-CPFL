...
def api_ged119():
    try: dados = request.get_json(); resultado = calc_ged119(dados); return jsonify({'success': True, 'data': resultado})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/calcular_transformador', methods=['POST'])
def api_calcular_transformador():
    try: dados = request.get_json(); resultado = calcular_transformador(dados); return jsonify({'success': True, 'data': resultado})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/calcular_poste', methods=['POST'])
def api_calcular_poste():
    try: dados = request.get_json(); resultado = calcular_poste(dados); return jsonify({'success': True, 'data': resultado})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/calcular_ramal', methods=['POST'])
def api_calcular_ramal():
    try: dados = request.get_json(); resultado = calcular_ramal(dados); return jsonify({'success': True, 'data': resultado})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/calcular_ramal_ligacao', methods=['POST'])
def api_calcular_ramal_ligacao():
    try: dados = request.get_json(); resultado = calcular_ramal_ligacao(dados); return jsonify({'success': True, 'data': resultado})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/calcular_neutro', methods=['POST'])
def api_calcular_neutro():
    try: dados = request.get_json(); fase = float(dados.get('fase_mm2', 0)); neutro = calcular_neutro(fase); return jsonify({'success': True, 'data': {'fase_mm2': fase, 'neutro_mm2': neutro}})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/tipos_eletroduto', methods=['GET'])
def api_tipos_eletroduto():
    try: tipos = get_tipos_eletroduto(); return jsonify({'success': True, 'data': tipos})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/calcular_eletroduto', methods=['POST'])
def api_calcular_eletroduto():
    try: dados = request.get_json(); resultado = calcular_eletroduto(dados['tipo_cabo'], float(dados['secao_mm2']), int(dados['n_condutores'])); return jsonify({'success': True, 'data': resultado})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/formas_agrupamento_nbr', methods=['GET'])
def api_formas_agrupamento_nbr():
    try: formas = get_formas_agrupamento_nbr(); return jsonify({'success': True, 'data': formas})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/formas_agrupamento', methods=['GET'])
def api_ged119_formas_agrupamento():
    try: formas = ged119_mod.get_formas_agrupamento(); return jsonify({'success': True, 'data': formas})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/tabela2', methods=['GET'])
def api_ged119_tabela2():
    try: conn = ged119_mod.get_conn(); cursor = conn.cursor(); cursor.execute("SELECT * FROM TABELA_2"); rows = cursor.fetchall(); cols = [d[0] for d in cursor.description]; conn.close(); data = [dict(zip(cols, r)) for r in rows]; return jsonify({'success': True, 'data': data})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged119/tabela_ac', methods=['GET'])
def api_ged119_tabela_ac():
    try: data = get_tabela_ac(); return jsonify({'success': True, 'data': data})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400
...
def api_ged13_tabela8():
    try: conn = get_conn_g13(); cursor = conn.cursor(); cursor.execute("SELECT DISTINCT btu_h, potencia_va, potencia_w FROM tabela8 ORDER BY btu_h"); rows = cursor.fetchall(); conn.close(); data = [{'btu': r[0], 'va': r[1], 'w': r[2]} for r in rows]; return jsonify({'success': True, 'data': data})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged13/sugestao', methods=['GET'])
def api_ged13_sugestao():
    try: area = float(request.args.get('area', 0)); tipo = request.args.get('tipo', 'Residencial'); resultado = get_sugestao(area, tipo); return jsonify({'success': True, 'data': resultado})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged13/calcular', methods=['POST'])
def api_ged13():
    try: dados = request.get_json(); resultado = calc_ged13(dados); return jsonify({'success': True, 'data': resultado})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

# --- PROJECT MANAGEMENT API ---
@app.route('/api/projetos/listar', methods=['GET'])
def api_projetos_listar():
    try: projetos = listar_projetos(); return jsonify({'success': True, 'data': projetos})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400
...
def api_projetos_salvar():
    try: dados = request.get_json(); projeto_id = dados.get('id'); novo_id = salvar_projeto(projeto_id, dados); return jsonify({'success': True, 'data': {'id': novo_id}})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/projetos/excluir/<int:projeto_id>', methods=['DELETE'])
def api_projetos_excluir(projeto_id):
    try: excluir_projeto(projeto_id); return jsonify({'success': True})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

# --- GED-13 PROJECT MANAGEMENT API ---
@app.route('/api/ged13/projetos/listar', methods=['GET'])
def api_ged13_projetos_listar():
    try: projetos = listar_projetos_g13(); return jsonify({'success': True, 'data': projetos})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400
...
def api_ged13_projetos_salvar():
    try: dados = request.get_json(); projeto_id = dados.get('id'); novo_id = salvar_projeto_g13(projeto_id, dados); return jsonify({'success': True, 'data': {'id': novo_id}})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/api/ged13/projetos/excluir/<int:projeto_id>', methods=['DELETE'])
def api_ged13_projetos_excluir(projeto_id):
    try: excluir_projeto_g13(projeto_id); return jsonify({'success': True})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 400
